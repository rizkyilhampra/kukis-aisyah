...
class AuthController extends Controller
{
    ...
    public function login()
    {
        ...
        if ($this->auth->check()) {
            // Cek Group/Role dari user yang telah login
            // kemudian redirect sesuai group/rolenya
            $authorize = service('authorization');
            helper('auth');
            if ($authorize->inGroup('user', user_id())) {
                $redirectURL =  site_url('user') ?? session('redirect_url');
                unset($_SESSION['redirect_url']);

                return redirect()->to($redirectURL)->withCookies()->with('message', lang('Auth.loginSuccess'));
            } else if ($authorize->inGroup('admin', user_id())) {
                $redirectURL =  site_url('admin') ?? session('redirect_url');
                unset($_SESSION['redirect_url']);
